...
import { Signer, AuthOptions, AUTH_TYPE, USER_AGENT_HEADER, USER_AGENT } from 'aws-appsync-auth-link'
import { GraphQLError, print } from 'graphql'
import * as url from 'url'
import { v4 as uuid } from 'uuid'
import { UrlInfo, SOCKET_STATUS, ObserverQuery, SUBSCRIPTION_STATUS, MESSAGE_TYPES } from './types'
...
export class AppSyncRealTimeSubscriptionHandshakeLink extends ApolloLink {
  ...
  request(operation: Operation) {
    ...
    return new Observable<FetchResult>((observer) => {
      ...
      const options = {
        ...
        jwtToken:
          this.auth.type === AUTH_TYPE.AMAZON_COGNITO_USER_POOLS || this.auth.type === AUTH_TYPE.OPENID_CONNECT
            ? this.auth.jwtToken
            : null,
      }
  ...
  private async _startSubscriptionWithAWSAppSyncRealTime({ options, observer, subscriptionId }) {
    ...
    try {
      ...
      const { subscriptionFailedCallback = null } = this.subscriptionObserverMap.get(subscriptionId) || {}
  ...
  private async _awsRealTimeHeaderBasedAuth({
    ...
    const headerHandler: Record<string, (info: any) => Promise<Record<string, string>>> = {
  ...
  private async _awsRealTimeOPENIDHeader({ host, jwtToken }): Promise<Record<string, string>> {
    return {
      Authorization: typeof jwtToken === 'function' ? await jwtToken.call(undefined) : await jwtToken,
      host,
    }
  }

  private async _awsRealTimeApiKeyHeader({ apiKey, host }): Promise<Record<string, string>> {
    const dt = new Date()
  ...
  private async _awsRealTimeIAMHeader({
    ...
    const signed_params = Signer.sign(request, formattedCredentials, endpointInfo)
  ...
  private async _initializeRetryableHandshake({ awsRealTimeUrl }) {
    ...
    await jitteredExponentialRetry(this._initializeHandshake.bind(this), [{ awsRealTimeUrl }])
  }
  ...
  private _timeoutStartSubscriptionAck(subscriptionId) {
    ...
    observer.error({
      errors: [
        {
          ...new GraphQLError(`Subscription timeout ${JSON.stringify({ query, variables })}`),
        },
      ],
    })
